...
import { injectGradingSheet } from "./gradingSheet.js";
...
export async function buildOutputZip({ studentResults, runMeta }) {
  ...
  // 3. Per-student folder with the original file (now with a "GRADING (auto)"
  //    sheet injected inside) + feedback.md + raw json
  for (const sr of studentResults) {
    const folder = zip.folder(sanitizePath(sr.studentName));
    if (sr.originalFile) {
      let xlsxBytes = sr.originalFile.bytes;
      try {
        xlsxBytes = await injectGradingSheet(xlsxBytes, sr, runMeta);
      } catch (err) {
        // If injection fails for any reason (corrupt xlsx, unusual layout),
        // fall back to the original bytes and keep going.
        folder.file(
          "GRADING_INJECTION_ERROR.txt",
          "Could not inject the GRADING (auto) sheet into the workbook:\n" +
            (err?.message ?? String(err))
        );
      }
      const baseName = stripExt(sr.originalFile.name);
      const ext = sr.originalFile.name.slice(baseName.length) || ".xlsx";
      folder.file(`${baseName}_GRADED${ext}`, xlsxBytes);
    }
