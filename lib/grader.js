...
export async function gradeWorkbook({
  ...
  // Hard cap to avoid runaway prompts. ~400KB ≈ 100K tokens, comfortable on
  // the 1M-context models we use. When we have to truncate we mark it clearly
  // so the model treats the suffix as "unavailable" rather than valid data.
  const CAP = 400_000;
  if (dumpText.length > CAP) {
    dumpText =
      dumpText.slice(0, CAP) +
      `\n\n/* TRUNCATED — workbook dump exceeded ${CAP} chars; later sheets/cells may be missing. ` +
      `If you cannot find an answer here, mark verdict="missing" rather than guessing. */`;
  }
  ...
  // The cacheable preamble: same for every student in a class. Anthropic's
  // prompt cache lets follow-up students pay ~10% of full input cost on this
  // block (5 min TTL). Cache write is +25% on the first student.
  const cacheablePreamble =
    `Grade ONE student's Excel workbook against the rubric below.\n\n` +
    `STRICTNESS: ${strictness.toUpperCase()}\n${STRICTNESS[strictness].description}\n\n` +
    `Search the ENTIRE workbook for each answer — not only the sheet or range mentioned in the question. ` +
    `If the student placed the answer in a different sheet, column, or row than the rubric expected, locate it and award points (mention where you found it in the feedback).\n\n` +
    `RUBRIC:\n\n${rubricText}`;

  const studentTail =
    `\n\nSTUDENT WORKBOOK (every relevant non-empty cell + structural metadata, JSON):\n\`\`\`json\n${dumpText}\n\`\`\`\n\n` +
    `Return ONE JSON object whose top-level keys are the question IDs (Q1, Q2, …, Q15). ` +
    `Each value: {points_awarded, verdict, evidence, feedback}. JSON only.`;
      ...
      messages: [
        {
          role: "user",
          content: [
            { type: "text", text: cacheablePreamble, cache_control: { type: "ephemeral" } },
            { type: "text", text: studentTail },
          ],
        },
      ],
      maxTokens: 8192,
...
async function gradePerQuestion({
  ...
  const dump = fullWorkbookDump(workbook);
  const dumpText = JSON.stringify(dump).slice(0, 300_000);
  const results = [];
...
export async function gradeSqlScreenshots({
  ...
  // Cacheable: strictness rules + reference tables + rubric (same for every
  // student in a class). Per-student variable: the screenshot images.
  const cacheablePreamble =
    `STRICTNESS LEVEL: ${strictness.toUpperCase()}\n${STRICTNESS[strictness].description}\n\n` +
    `REFERENCE TABLES (the source-of-truth data the student worked with):\n${referenceTables}\n\n` +
  ...
  const messages = [
    {
      ...
      content: [
        ...
        })),
        {
          type: "text",
          text:
            `Match each screenshot to the appropriate question. ` +
            `Return a single JSON object of the form ` +
            `{ "Q16": {points_awarded, verdict, evidence, feedback}, "Q17": {...}, ..., "Q20": {...} }. ` +
            `Output JSON only.`,
        },
      ],
    },
