...
export async function injectGradingSheet(xlsxBytes, studentResult, runMeta) {
  ...
  // 1. Fill D6:D26 with the per-question grade on a 0–10 scale, matching the
  //    template (each D cell defaults to 10 = full credit, e.g. a student who
  //    earned 1.5 out of Q1's 2 points gets 7.5 in D6).
  ...
  // 2. Update D27 (TOTAL) cached value. With D on a 0–10 scale and weights
  //    that sum to 1.0, SUMPRODUCT(C, D) lands on the same 0–10 total the
  //    template was designed for.
  const totalOutOf10 = round2((studentResult.totalPoints || 0) / 10);
  sheetXml = updateTotalCell(sheetXml, totalOutOf10);
  ...
  const lastRow = 29 + QUESTIONS.length; // header row 29 + one per question
...
function buildJustificationRows(sr /* runMeta unused — kept out of the sheet */) {
  const out = [];
  // Row 28 left blank as a visual separator from the totals row.
  out.push(`<row r="28" spans="2:7"/>`);
  // Row 29: plain column headers (no AI-tool branding).
  out.push(
    `<row r="29" spans="2:7">` +
      inlineCell(`B29`, `Question`) +
      inlineCell(`C29`, `Max points`) +
      inlineCell(`D29`, `Awarded`) +
      inlineCell(`E29`, `Verdict`) +
      inlineCell(`F29`, `Evidence`) +
      inlineCell(`G29`, `Feedback`) +
      `</row>`
  );
  // Rows 30+: one per question
  for (let i = 0; i < QUESTIONS.length; i++) {
    const q = QUESTIONS[i];
