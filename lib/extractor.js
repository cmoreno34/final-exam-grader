...
// every sheet so Claude can locate answers wherever the student put them.
// Caps the JSON size by truncating the data block of section 2 (which can
// be hundreds of pure-data rows that don't help grading).
export function fullWorkbookDump(workbook, options = {}) {
  const maxBodyRows = options.maxBodyRows ?? 60; // cap repeated data rows
  const out = {
    definedNames: workbook.definedNames,
  ...
  for (const [name, sh] of Object.entries(workbook.sheets)) {
    const cells = {};
    // Pure data sheets (section 2_DATA, sect 2 quesion 10) can have hundreds
    // of non-empty rows of plain values. Keep header + first N data rows so
    // Claude sees the schema, drop the rest.
    const isBulkData = /section\s*2_DATA|sect\s*2\s*quesion\s*10/i.test(name);
    const cap = isBulkData ? maxBodyRows : sh.maxRow;
    for (const [ref, cell] of Object.entries(sh.cells)) {
      if (cell.f === undefined && cell.v === undefined) continue;
      const m = /^([A-Z]+)(\d+)$/.exec(ref);
      if (!m) continue;
      const row = parseInt(m[2], 10);
      if (row > cap + 1) continue; // +1 for header
      const c = {};
    ...
    out.sheets[name] = {
      ...
      truncatedTo: isBulkData ? maxBodyRows : null,
    };
  }
  return out;
}
