...
export function fullWorkbookDump(workbook, options = {}) {
  ...
  const dataSampleRows = options.dataSampleRows ?? 5;     // section 2 source data: 5 sample rows
  const consolidatedSampleRows = options.consolidatedSampleRows ?? 3; // Q10 result table: 3 sample rows
  const chartSheetMaxRow = options.chartSheetMaxRow ?? 80; // section 2 charts page: top of sheet only
  const out = {
    definedNames: workbook.definedNames,
